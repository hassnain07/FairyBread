...
interface TeacherMessagesViewProps { teacherId: string; }

export function TeacherMessagesView({ teacherId }: TeacherMessagesViewProps) {
  ...
  const { data: conversations = [] } = useQuery({
    ...
    queryFn: () => dataClient.getConversations(teacherId),
  });
  ...
  useEffect(() => {
    if (convId) {
      dataClient.markConversationRead(convId).then(() =>
        qc.invalidateQueries({ queryKey: ['conversations', teacherId] })
      );
    }
  }, [convId, teacherId, qc]);
  ...
  const otherName = activeConv?.participant_names.find((_, i) => activeConv.participant_roles[i] !== 'teacher') ?? '';
    ...
    <div className="page-stack messages-layout">
      <div className="card conversation-list">
        ...
        {conversations.map(conv => {
          const other = conv.participant_names.find((_, i) => conv.participant_roles[i] !== 'teacher') ?? conv.participant_names[0];
...
export function TeacherMessagesPage() {
  return <TeacherMessagesView teacherId="tch1" />;
}
